...
class TestTlsVerify:
    async def test_verify_string_resolved_to_ssl_context(self) -> None:
        ...
        Path resolution walks up from the test file looking for a
        ``certs/ca.crt`` so the test works both in the dev container
        (``/app/tests/test_ml_client.py``) and in the host checkout
        (``<repo>/apps/estimator-api/tests/test_ml_client.py``).
        """
        import ssl
        ...
        for ancestor in Path(__file__).resolve().parents:
            candidate = ancestor / "certs" / "ca.crt"
            if candidate.exists():
                ca_path = candidate
                break
        if ca_path is None:
