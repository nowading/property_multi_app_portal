...
/**
 * Unit tests for {@link CsvDatasetPort}.
 */
@DisplayName("CsvDatasetPort")
class CsvDatasetPortTest {

    private CsvDatasetPort port;

    @BeforeEach
    void setUp() {
        port = new CsvDatasetPort();
        port.init();
    }

    @Test
    @DisplayName("should load all rows from CSV")
    void findAll() {
        List<PropertyRow> rows = port.findAll();
    ...
    void filterByBedroomsMin() {
        ...
        List<PropertyRow> filtered = port.findByFilters(filters);
    ...
    void findPage() {
        ...
        DatasetPage page1 = port.findPage(emptyFilters, 1, 10);
        ...
        DatasetPage page6 = port.findPage(emptyFilters, 6, 10);
    ...
    void countByFilters() {
        ...
        long count = port.countByFilters(filters);
    ...
    void noMatches() {
        // Use a very small price max that won't match any row
    ...
    @Test
    @DisplayName("should parse CSV row correctly")
    void parseRow() {
        List<PropertyRow> rows = port.findAll();
